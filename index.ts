...
type ExtractErrFromRecordSource<T extends Record<string, Observable<Result<any, any>>>> =
    T[keyof T] extends Observable<Result<any, infer E>> ? E : never;
type ExtractOkFromRecordSource<T extends Record<string, Observable<Result<any, any>>>> = {
...
export function forkJoinResult<T extends Record<string, Observable<Result<any, any>>>>(
    sources: T,
): Observable<Result<Prettify<ExtractOkFromRecordSource<T>>, ExtractErrFromRecordSource<T>>>;
export function forkJoinResult<T extends Observable<Result<any, any>>[]>(
    sources: T,
): Observable<Result<ExtractOkFromArraySource<T>, ExtractErrFromArraySource<T>>>;
export function forkJoinResult<T>(sources: T): any {
    const entries = Object.entries(sources as any) as [string, Observable<Result<any, any>>][];
